...
- [x] ~~**#65** `RunPush` silent-stop reconnect never backs off~~ `#bug` `#poplar` `#sync` *(2026-08-18, pass 1b round-3 re-review)* (closed 2026-08-18)
  Resolved by pass 1c task 2. `pushState.stopped` now advances `attempt` on a silent stop exactly as `fail` does on a Listen error, and resets it (alongside `silent`) on a delivered notification; `proved()` already reset `attempt`, unchanged. `RunPush`'s tail `sleepBackoff` call takes `push.attempt` instead of a fixed `0`. The pinning test that asserted the non-escalating floor, `TestRunPushDoesNotEscalateOnAnUnexplainedStop`, is replaced by `TestPushStateStoppedAdvancesOnSilence` (the escalation/reset contract directly on `pushState`) and `TestRunPushEscalatesOnASilentStop` (the same curve through `RunPush`). The ADR-0005 poll-fallback timing (`TestRunPushPollsWhileTheStreamStaysRefused`) passed unmodified.
