...
| Address book — `internal/ui/contacts/` package; fixture-backed; popover, T9 sidebar + List + RenderDetailCard for Contacts mode (9.1a); `Form` (Person/Business, two render contexts, focusList model, `Ctrl+S` validate, `Esc`/dirty-confirm via `pendingFormDiscard`) (9.1b); compose autocomplete `Dropdown` + `SuggestFn` seam threaded through `compose.New`/`Open`, fixture-backed via `contacts.FixtureSuggestions`, splices below the focused header (9l); 9m CardDAV swap | 0166, 0167, 0174 |
