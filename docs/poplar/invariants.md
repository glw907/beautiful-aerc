...
- Sidebar renders three folder groups in fixed order: Primary,
  ...
  Groups are permanent — user config only ranks folders within their
  group.
...
- Modifier-free: user-facing actions never bind a Ctrl/Alt/Meta
  chord. Viewer scroll uses `j/k/Space/b/g/G`. `Ctrl-c` survives
  only as a terminal-kill alias on the Quit binding; never
  advertised. `pgup/pgdown` are not bound.
- Folder jumps use uppercase single keys: `I` Inbox, `D` Drafts,
  `S` Sent, `A` Archive, `X` Spam, `T` Trash. Shared with lowercase
  triage keys (`d` delete vs `D` drafts) without conflict.
