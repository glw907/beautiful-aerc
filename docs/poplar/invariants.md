...
| Drafts persistence (JMAP) — schema v7 `drafts` (server_uid/server_folder paired by CHECK), gob payload via `compose.EncodeDraft`/`DecodeDraft`; `KindPushDraft` outbox op with payload-bearing `QueuePushDraft`; `mail.Backend.PushDraft` + `mail.ErrUnsupported`; JMAP one-request `Email/import` + `Email/set destroy` (atomic, last-write-wins, notFound on destroy benign); IMAP stub returns ErrUnsupported (App gates on `IsJMAP()`); `compose.Model` lifecycle — `CacheStore` test seam, 1s autosave debounce, 5min server push tick, `Open()`/`SetCache`/`SetDraftTarget`; App routes Drafts-folder Enter through `LookupDraftByServerUID` (server-pushed) or `draft:<id>` synthetic UIDs from `QueryFolder` (local-only); Ctrl+C dirty → Save/Discard/Keep modal | 0164 |
