...
**Current pass:** Pass 9k.2 next — cache + outbound chain comment sweep.
...
## Next starter prompt (Pass 9k.2)

> **Goal.** Second slice of the comment voice sweep: cache +
> outbound chain. `internal/cache/` (~344 cmt, 14.0% — largest
> single package), `internal/compose/`, `internal/content/`,
> `internal/filter/` (21.3%), `internal/tidy/`, plus the tinies
> (`internal/humanize/`, `internal/backoff/`, `internal/theme/`).
> ~788 cmt total.
>
> **Method.** Same as 9k.1: §0(a)/(b)/(c) gate plus the paraphrase
> test on every in-function comment, T39/T40 shape on every godoc,
> ≤1 ADR/RFC cite per godoc, no new comments, commit per-package
...
> packages is locked (ADR-0170). Cache architecture is locked
> (ADRs 0110–0124, 0132–0134, 0158); compose foundation is locked
> (ADR-0156); drafts persistence is locked (ADRs 0164, 0165).
>
> **Still open.** None — pure implementation pass. If a new
> recurring shape surfaces, log it as T41+ in a follow-up ADR.
>
> **Approach.** Write `docs/superpowers/plans/YYYY-MM-DD-comment-
> sweep-cache-outbound.md` with one task per package. Standard
