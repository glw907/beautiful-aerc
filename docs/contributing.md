...
beautiful-aerc/
  ...
  internal/filter/
    ...
    html.go                  HTML-to-markdown conversion, cleanup, Glamour rendering
...
The repo builds three binaries from one Go module (mailrender, fastmail-cli, tidytext). The boundary between `cmd/` and `internal/` is strict: `cmd/` contains only cobra wiring and flag parsing. All logic lives in `internal/`. This makes functions directly testable without subprocess invocation.
...
1. Load theme
2. Strip HTML artifacts (moz attributes, hidden divs, tracking pixels) - `prepareHTML`
3. Convert HTML to markdown via `html-to-markdown` Go library (`convertHTML`) with layout table flattening and image stripping plugins
4. Normalize whitespace, deduplicate blocks, strip empty links, collapse short blocks
5. Unflatten inline quote markers from Outlook-style emails (`unflattenQuotes`)
6. Reflow paragraphs and blockquotes to fixed width (`reflowMarkdown`)
7. Render styled output via Glamour with theme-derived styles (`GlamourStyle`)
8. Write to stdout
