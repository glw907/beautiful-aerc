...
import (
	...
	"github.com/glw907/beautiful-aerc/internal/aercfork/keepalive"
...
func (w *IMAPWorker) setKeepaliveParameters(conn *net.TCPConn) error {
	...
	err = rawConn.Control(func(fdPtr uintptr) {
		...
		err := keepalive.SetTcpKeepaliveProbes(fd, w.config.keepalive_probes)
		if err != nil {
			w.worker.Errorf("cannot set tcp keepalive probes: %v", err)
		}
		// Wait time after an unsuccessful probe
		err = keepalive.SetTcpKeepaliveInterval(fd, w.config.keepalive_interval)
		if err != nil {
