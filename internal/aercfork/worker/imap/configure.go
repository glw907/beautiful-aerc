...
import (
	...
	"github.com/glw907/beautiful-aerc/internal/aercfork/worker/lib"
	"github.com/glw907/beautiful-aerc/internal/aercfork/worker/middleware"
...
func (w *IMAPWorker) handleConfigure(msg *types.Configure) error {
	...
	if strings.HasSuffix(w.config.scheme, "+oauthbearer") {
		...
		if q.Get("token_endpoint") != "" {
			oauth2cfg.ClientID = q.Get("client_id")
			oauth2cfg.ClientSecret = q.Get("client_secret")
			oauth2cfg.Scopes = []string{q.Get("scope")}
			oauth2cfg.Endpoint.TokenURL = q.Get("token_endpoint")
		}
		w.config.oauthBearer.OAuth2 = oauth2cfg
	}
	...
	if strings.HasSuffix(w.config.scheme, "+xoauth2") {
		...
		w.config.xoauth2.OAuth2 = oauth2cfg
	}
