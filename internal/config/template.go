...
# ──────────────
#
# IMAP-backed accounts need an SMTP submission server to send
# mail. Preset providers fill these in automatically; you only
# need an [account.smtp] block when overriding defaults or
# configuring a self-hosted server.
#
# JMAP accounts (fastmail) ignore [account.smtp] — submission
# rides the existing JMAP session.
#
# When [account.smtp] is omitted, SMTP credentials default to
# the IMAP-side ` + "`password`" + ` / ` + "`password-cmd`" + ` and ` + "`auth`" + ` — the
# typical case is "same login as IMAP."
#
# [account.smtp]
...
# starttls     = false
# insecure-tls = false
# auth         = "plain"    # plain, login, xoauth2
# password-cmd = "..."      # only when SMTP creds differ from IMAP
...
#
# undo-send-window = "10s"
#     Hold before queued mail is dispatched; shows an undo banner.
#     "0s" disables the hold (immediate dispatch). Range 0s-5m.
#
...
# [cache]
# max-size = "0"
#     Body-cache size cap. "0" (the default) is unlimited; set to
#     a value like "5GB" to evict oldest-by-sent-date bodies when
#     total exceeds the cap. Suffixes: KB, MB, GB, TB (1024-based).
`
