...
func TestConflictOverlay_CursorClampedNoWrap(t *testing.T) {
	...
	o2, _ := o.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'k'}})
	if o2.cursor != 0 {
		t.Errorf("k at top: cursor want 0, got %d", o2.cursor)
	}

	// j: cursor advances 0→1
	o3, _ := o2.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'j'}})
	if o3.cursor != 1 {
		t.Errorf("j: cursor want 1, got %d", o3.cursor)
	}

	// j at bottom: cursor stays 1
	o4, _ := o3.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'j'}})
	if o4.cursor != 1 {
...
func TestConflictOverlay_BangAndEscClose(t *testing.T) {
	...
	o2, _ := o.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'!'}})
	if o2.IsOpen() {
	...
	o4, _ := o3.Update(tea.KeyMsg{Type: tea.KeyEsc})
	if o4.IsOpen() {
