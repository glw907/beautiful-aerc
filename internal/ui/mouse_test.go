...
// TestPaneAt proves paneAt resolves a point against LayoutMode's own
// pane rectangles (ADR-0017's pane grain), and reports false for a
// point outside every pane (a chrome band, most notably).
...
// given digit span back to that digit's own physical key, index for
// index against GrammarKeys.SurfaceSwitch.Keys(), not the shared
...
// (specialKeyPresses) and a printable literal (the fallback), for
// every key GrammarKeys.fields() actually binds as a first key.
...
func TestResolveDoubleClick_WithinWindowUpgrades(t *testing.T) {
	...
	prev, double := resolveDoubleClick(pendingClick{}, target, opened)
	if double {
...
func TestResolveDoubleClick_OutsideWindowSelectsTwice(t *testing.T) {
	target := "row-1"
	opened := time.Date(2026, 8, 20, 12, 0, 0, 0, time.UTC)
	prev, _ := resolveDoubleClick(pendingClick{}, target, opened)
...
// own test, not just timing.
func TestResolveDoubleClick_DifferentTargetNeverUpgrades(t *testing.T) {
	opened := time.Date(2026, 8, 20, 12, 0, 0, 0, time.UTC)
	prev, _ := resolveDoubleClick(pendingClick{}, "row-1", opened)
...
// the character-grain dispatch's own first stop: clicking a status
// digit's own rendered cell switches surfaces exactly as pressing that
...
func TestApp_ClickStatusDigit_SwitchesSurfaceInDigitsSwitchState(t *testing.T) {
	...
	click := tea.MouseClickMsg{X: spans[SurfaceContacts].Rect.Min.X, Y: spans[SurfaceContacts].Rect.Min.Y}
...
// footer hint's own cell fires exactly the Cmd pressing its key would:
...
func TestApp_ClickFooterHint_RunsItsVerb(t *testing.T) {
	...
	spans := FooterHitSpans(front, front.SwitchState, app.layout.Width, app.layout.Footer.Rect)
	...
	click := tea.MouseClickMsg{X: quit.Rect.Min.X, Y: quit.Rect.Min.Y}
...
// own dismiss glyph clears it, the same outcome Esc already produces
...
func TestApp_ClickBannerDismiss_Dismisses(t *testing.T) {
	...
	click := tea.MouseClickMsg{X: spans[0].Rect.Min.X, Y: spans[0].Rect.Min.Y}
	app = mustApp(t, first(app.Update(click)))
...
// own y/n cell answers exactly as the key would, through the same
// ConfirmAnsweredMsg round trip TestApp_ConfirmOnStack_AnswersYesNoAndPops
// drives with a real keypress.
func TestApp_ClickConfirmAnswer_Answers(t *testing.T) {
	...
	click := tea.MouseClickMsg{X: yes.Rect.Min.X, Y: yes.Rect.Min.Y}
...
// on neither a hit span nor a pane (past the terminal's own bounds,
// here) leaves App's rendered view byte-for-byte unchanged: pointer
// input changes state, never rendering rules (ADR-0017's testing
// section).
func TestApp_NoOpClickLeavesTheGoldenUnchanged(t *testing.T) {
	...
	click := tea.MouseClickMsg{X: 999, Y: 999}
	app = mustApp(t, first(app.Update(click)))
